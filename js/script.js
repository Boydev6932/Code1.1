...
(() => {
  ...
  /* --------------------------------------------------------------------------
     ติด มผ. ตาม “ดัชนีหลัง clean” ที่คุณกำหนด (ตัวเลขต้องตรง 100%)
     -------------------------------------------------------------------------- */
  const needFixIndexes = [0,1,4,7,8,10,11,14,17,19,23,27,28,29,31,49,57,59,64,65,72,75,77,84,87,101,106,115,116,117]
    .filter(i => i < names.length);

  /* --------------------------------------------------------------------------
     STRICT SELF-CHECK: แมป (index ↔ ชื่อ) ที่คุณยืนยัน เพื่อกันกรณีลำดับเปลี่ยน
     - หากชื่อในดัชนีที่กำหนดไม่ตรง ระบบจะ “เสริม” ด้วยการจับจากชื่อจริง
       ทำให้ยังขึ้น “ติด มผ.” ได้ถูกต้อง 100% แม้ลำดับขยับ
     -------------------------------------------------------------------------- */
  const strictIndexNamePairs = [
    [0,"กบิลพัสดุ์ แสงชัย"], [1,"แทนคุณ จันงาม"], [4,"จิรัชยานันท์ แข็งขยัน"], [7,"ทิวากร ฉัตรานุฉัตร"],
    [8,"พงศกร ขำตา"], [10,"ปัณณวัฒน์ ไทยวังชัย"], [11,"ภูมิภัทร สามารถกุล"], [14,"สหัสวรรษ มหาปัญญาวงศ์"],
  ...
  // สร้าง Set สุดท้ายที่ “กันพลาด” จากทั้งดัชนีและชื่อจริง (union)
  const needFixSet = (() => {
    const byIndex = new Set(needFixIndexes);
    const byNameSet = new Set(strictIndexNamePairs.map(([,nm]) => nm));
    const nameToIndex = new Map(names.map((n, i) => [n, i]));

    // รวม index ที่ระบุไว้
    const final = new Set(byIndex);

    // เสริม index ที่หาได้จาก “ชื่อจริง” (กันกรณีลำดับเปลี่ยน)
    for (const nm of byNameSet) {
      const idx = nameToIndex.get(nm);
      if (typeof idx === 'number') final.add(idx);
    }

    // self-check ใน console (ไม่กระทบผู้ใช้): เตือนถ้าดัชนีกับชื่อไม่ตรง
    try {
